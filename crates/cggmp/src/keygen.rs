...
impl<'a, I: IdentityProvider> AugmentedStateMachine for AugmentedKeyGen<'a, I> {
    ...
    fn pre_handle_incoming(
        ...
        match &msg.body.base.0 {
            ...
            M::Round1(out_msg) => match msg.body.extra.as_ref() {
                Some(params) => {
                    // Verifies that signer is an expected party/signatory.
                    if !self.parties.contains(&params.verifying_key) {
    ...
    ) -> Result<Option<Self::AdditionalParams>, Error<<Self::StateMachineType as StateMachine>::Err>>
    {
        match &msg_body.0 {
            ...
            M::Round1(out_msg) => Ok(Some(IdentityAuthParams {
                verifying_key: self.identity_provider.verifying_key(),
                verifying_signature: self.identity_provider.sign(
                    &wamu_core::utils::prefix_message_bytes(&Self::parameter_hash(sender, out_msg)),
                ),
            })),
            // No modifications for other rounds.
            _ => Ok(None),
